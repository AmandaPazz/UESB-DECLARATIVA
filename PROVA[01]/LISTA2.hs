...
-- 1 = Denir uma fun��o para somar todos os pares de 1 at� N.
somaPares :: Int -> Int
somaPares 0 = 0
somaPares n
...
-- 2. Fun��o para calcular x elevado a y (x^y)
...
-- 3. Fun��o para somar todos os n�meros em um intervalo entre A e B, inclusive
...
-- 4. Fun��o para somar os d�gitos de um n�mero inteiro positivo N
somaDigitos :: Int -> Int
somaDigitos 0 = 0
somaDigitos n
...
-- 5. Fun��o para somar os quadrados de todos os inteiros de 0 a N
...
-- 6. Fun��o para somar os fatoriais de todos os inteiros de 0 a N
...
-- 7. Fun��o para calcular uma aproxima��o de ? usando a s�rie de Leibniz
approxPi :: Integer -> Double
approxPi 0 = 4.0
approxPi n = 4 * ((-1) ^ n / fromIntegral (2 * n + 1)) + approxPi (n - 1)

-- 8. Fun��o para somar dois n�meros utilizando a fun��o sucessor
somaComSucessor :: Int -> Int -> Int
somaComSucessor x y
  | y == 0 = x
  | otherwise = somaComSucessor (succ x) (pred y)

-- 9. Fun��o para multiplicar dois n�meros utilizando a fun��o soma
multiplicaComSoma :: Int -> Int -> Int
multiplicaComSoma x y
  | y == 0 = x
  | otherwise = x + multiplicaComSoma x (y - 1)

-- 10. Fun��o que determina quantas vezes um d�gito K ocorre em um n�mero natural N
...
-- 11. Fun��o para converter um n�mero decimal em bin�rio
...
-- 12. Fun��o para concatenar uma palavra N vezes
concatenaPalavra :: String -> Int -> String
concatenaPalavra _ 0 = ""
concatenaPalavra palavra n = palavra ++ concatenaPalavra palavra (n - 1)

-- 13. Explica��o sobre recurs�o de cauda
-- Recurs�o de cauda ocorre quando a chamada recursiva � a �ltima coisa que acontece na fun��o.
-- Isso permite otimiza��es no tempo de execu��o, evitando a cria��o de novas pilhas de chamadas.

-- 14. Fun��o bonacci que calcula o n-�simo n�mero da sequ�ncia de Fibonacci usando recurs�o de cauda
fibonacci :: Int -> Int
...
-- 15. Fun��o para calcular o m�ximo divisor comum entre dois n�meros usando o algoritmo de Euclides
mdc :: Int -> Int -> Int
mdc a 0 = a
mdc a b = mdc b (a `mod` b)

-- 16. Fun��o para receber um n�mero N e devolver a tabuada de N
tabuada :: Int -> [String]
...
-- 17. Fun��o para calcular o valor de e com n termos usando a s�rie de Taylor
calculaE :: Int -> Double
calculaE 0 = 1.0
calculaE n = 1.0 / fromIntegral (fatorial n) + calculaE (n-1)

-- 18. Fun��o para montar uma tabela com os valores de e utilizando de 1 at� t termos
tabelaE :: Int -> [(Int, Double)]
tabelaE t = [(i, calculaE i) | i <- [1..t]]

-- 19. Fun��o para calcular a soma dos primeiros n termos da s�rie de Taylor para o seno
taylorSin :: Double -> Integer -> Double
taylorSin _ 0 = 0.0
taylorSin x n = ((-1) ^ n) * x^(2 * n + 1) / fromIntegral (fatorial (2 * fromIntegral n + 1)) + taylorSin x (n-1)

-- 20. Fun��o para calcular a soma dos primeiros n termos da s�rie de Taylor para o cosseno
taylorCos :: Double -> Integer -> Double
taylorCos _ 0 = 1.0
taylorCos x n = ((-1) ^ n) * x^(2 * n) / fromIntegral (fatorial (2 * fromIntegral n)) + taylorCos x (n-1)

-- 21. Fun��o para calcular a m�dia das vendas e o desvio padr�o
tamanhoLinha :: Int
tamanhoLinha = 30
...
-- Fun��es para calcular m�dia e desvio padr�o
...
-- Definindo a fun��o recursiva para c�lculo da soma das vendas
